...
static struct opt_table opt_config_table[] =
{
    ...
    OPT_WITH_ARG("--bitmain-freq",
                 set_int_0_to_9999,opt_show_intval, &opt_bitmain_B3_freq,
                 "Set frequency"),
...
)
{
    ...
    if (json_is_true(res)|| (ALGO_BTM && json_is_null(err)) || (work->gbt && json_is_null(res)))
    {
...
static bool test_work_current(struct work *work)
{
    ...
    if (!block_exists(hexstr))
    {
        ...
        if (work->longpoll)
        {
            if (work->stratum)
            {
                applog(LOG_DEBUG, "Stratum from pool %d detected new block",
                       pool->pool_no);
            }
            else
            {
                applog(LOG_DEBUG, "%sLONGPOLL from pool %d detected new block",
                       work->gbt ? "GBT " : "", work->pool->pool_no);
            }
        }
        else if (have_longpoll && !pool->gbt_solo)
            applog(LOG_DEBUG, "New block detected on network before pool notification");
        else if (!pool->gbt_solo)
            applog(LOG_DEBUG, "New block detected on network");
        restart_threads();
    }
    else
    {
        ...
        if (work->longpoll)
        {
            ...
            if (shared_strategy() || work->pool == current_pool())
            {
                if (work->stratum)
                {
                    applog(LOG_DEBUG, "Stratum from pool %d requested work restart",
                           pool->pool_no);
                }
                else
                {
                    applog(LOG_DEBUG, "%sLONGPOLL from pool %d requested work restart",
                           work->gbt ? "GBT " : "", work->pool->pool_no);
                }
                restart_threads();
            }
...
static void hashmeter(int thr_id, uint64_t hashes_done)
{
    ...
    if (thr_id >= 0)
    {
        ...
        thr_mhs = (double) (hashes_done / device_tdiff );
...
static bool parse_stratum_response(struct pool *pool, char *s)
{
    ...
    if (0)//json_is_null(id_val) || !id_val)
    {
        char *ss;
...
static void *stratum_sthread(void *userdata)
{
    ...
    while (42)
    {
        char noncehex[33], nonce2hex[20], s[1024];
        struct stratum_share *sshare;
        ...
        if (1)
        {
            ...
            sshare->work = work;
			uint64_t tmp_nonce = htobe64(work->btm_nonce);
            char *ASCIINonce = bin2hex((const unsigned char *)&tmp_nonce, 8);
            ...
            snprintf(s, sizeof(s), "{\"method\": \"submit\", \"params\": {\"id\": \"%s\", \"job_id\": \"%s\", \"nonce\": \"%s\"}, \"id\":%d}", pool->BTMAuthID, work->job_id, ASCIINonce, sshare->id);
...
retry_stratum:
    ...
    {
        applog(LOG_DEBUG, "No Stratum, GBT or Solo support in pool %d %s unable to use", pool->pool_no, pool->rpc_url);
        return false;
    }
...
void gen_stratum_work(struct pool *pool, struct work *work)
{
    ...
    work->btm_target = pool->btm_tgt;
	target_tmp = pool->btm_tgt;
    while(target_tmp)
    {
        target_tmp >>= 1;
        n++;
    ...
    {
        char *header;
        header = bin2hex(work->data, (size_t)136);
...
bool submit_nonce_direct(struct thr_info *thr, struct work *work, uint64_t nonce)
{
    struct work *work_out;
    uint64_t *work_nonce = (uint64_t *)(work->data + 128);
    *work_nonce = htole64(nonce);
	work->btm_nonce = nonce;
    work_out = copy_work(work);
    submit_work_async(work_out);
    return true;
}
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        cg_memcpy(g_miner_type, tmp, start-tmp);
                    }
                }
