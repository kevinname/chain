...
static size_t resp_hdr_cb(void *ptr, size_t size, size_t nmemb, void *user_data)
{
    ...
    if (opt_protocol)
        applog(LOG_NOTICE, "HTTP hdr(%s): %s", key, val);
                      ...
                      struct pool *pool, bool share)
{
    ...
    if (opt_protocol)
        applog(LOG_NOTICE, "JSON protocol request:\n%s", rpc_req);
    ...
    *rolltime = hi.rolltime;
    ...
    if (!val)
    {
        ...
        if (opt_protocol)
            applog(LOG_NOTICE, "JSON protocol response:\n%s", (char *)(all_data.buf));
...
out:
    if (!sret)
        ...
        applog(LOG_NOTICE, "RECVD: %s", sret);
    return sret;
}
...
static bool parse_notify(struct pool *pool, json_t *val)
{
    ...
    if (merkles)
    {
        ...
        for (i = 0; i < merkles; i++)
        {
            ...
            if (opt_protocol)
                applog(LOG_NOTICE, "merkle %d: %s", i, merkle);
...
out_unlock:
    ...
    if (opt_protocol)
    {
        ...
        applog(LOG_NOTICE, "prev_hash: %s", prev_hash);
        applog(LOG_NOTICE, "coinbase1: %s", coinbase1);
        applog(LOG_NOTICE, "coinbase2: %s", coinbase2);
        applog(LOG_NOTICE, "bbversion: %s", bbversion);
        applog(LOG_NOTICE, "nbit: %s", nbit);
        applog(LOG_NOTICE, "ntime: %s", ntime);
...
static bool parse_notify_btm(struct pool *pool, json_t *val)
{
    json_t *job_id, *version, *height, *prev_hash, *merkle_root,*status_hash, *seed, *nbit,
         *ntime, *nonce, *target;
	
	char header[274],seed_hex[66];
	uint8_t tmp_seed[32] = {0};
    uint32_t btm_target;
	bool ret;

	job_id = json_object_get(val, "job_id");
	version = json_object_get(val, "version");
	height = json_object_get(val, "height");
	prev_hash = json_object_get(val, "previous_block_hash");
	merkle_root = json_object_get(val, "transactions_merkle_root");
	status_hash = json_object_get(val, "transaction_status_hash");
	seed = json_object_get(val, "seed");
	nbit = json_object_get(val, "bits");
	ntime = json_object_get(val, "timestamp");
	nonce = json_object_get(val, "nonce");
	target = json_object_get(val, "target");

	hex2bin(&btm_target, json_string_value(target), 4);
    cg_wlock(&pool->data_lock);
	...
	if (job_id != NULL)
	{
    	pool->swork.job_id =  strdup(json_string_value(job_id));
		applog(LOG_NOTICE,"%s %s",json_string_value(job_id),pool->swork.job_id);
	}
	pool->btm_tgt = btm_target;
    snprintf(header, 273,
             "%s%s%s%s%s%s%s%s",
             json_string_value(version),
             json_string_value(height),
             json_string_value(prev_hash),
             json_string_value(ntime),
             json_string_value(merkle_root),
             json_string_value(status_hash),
             json_string_value(nbit),
             json_string_value(nonce)
            );
    ret = hex2bin(pool->header_bin, header, (size_t)136);
    if (unlikely(!ret))
    {
        applog(LOG_ERR, "Failed to convert header to header_bin in parse_notify_btm %s",header);
        goto out_unlock;
    }
	snprintf(seed_hex, 65, "%s", json_string_value(seed));
	
	ret = hex2bin(tmp_seed, json_string_value(seed), (size_t)32);
	if(memcmp(pool->btm_seed,tmp_seed,32) != 0)
		pool->update_seed = true;
	ret = hex2bin(pool->btm_seed, json_string_value(seed), (size_t)32);
    if (unlikely(!ret))
    {
        applog(LOG_ERR, "Failed to convert seed to seed_bin in parse_notify_btm");
        goto out_unlock;
    }
...
bool parse_method(struct pool *pool, char *s)
{
    ...
    if (1)
    {
        ...
        {
            if (parse_notify_btm(pool, params))
            {
                pool->stratum_notify = ret = true;
            }
            else
...
{
    json_t *val = NULL, *res_val, *err_val, *res_job, *res_id;
    char s[RBUFSIZE], *sret = NULL;
    json_error_t err;
    bool ret = false;
#if 0
	...
	if (1)
    {
        ...
        {
            cg_wlock(&pool->data_lock);
            strcpy(pool->BTMAuthID, json_string_value(res_id));
            applog(LOG_DEBUG, "BTM AuthID: %s", pool->BTMAuthID);
            cg_wunlock(&pool->data_lock);

            //get the job object and send to parse notify
            if ((res_job = json_object_get(res_val, "job")))
            {
                if (parse_notify_btm(pool, res_job))
                {
...
out:
    if (ret)
    {
        ...
        if (opt_protocol)
        {
            applog(LOG_NOTICE, "Pool %d confirmed mining.subscribe with extranonce1 %s extran2size %d", pool->pool_no, pool->nonce1, pool->n2size);
        }
    }
    else
    {
