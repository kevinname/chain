...
struct pool
{
    ...
    int nonce2_offset;
    unsigned char header_bin[136];
	unsigned char btm_seed[32];
	bool update_seed;
    int merkles;
    char prev_hash[68];
    char bbversion[12];
...
extern bool submit_nonce_direct(struct thr_info *thr, struct work *work, uint64_t nonce);
extern bool submit_noffset_nonce(struct thr_info *thr, struct work *work, uint32_t nonce, int noffset);
extern struct work *get_work(struct thr_info *thr, const int thr_id);
extern struct work *make_work(void);
extern void gen_stratum_work(struct pool *pool, struct work *work);
