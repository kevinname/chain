...
struct Mat256x256float {
  ...
  void copyFrom(const Mat256x256float& other) {
    for(int i=0; i<256; i++) {
      for(int j=0; j<256; j++) {
        this->f[j][i]=other.f[j][i];
      }
    }
  }
  void copyFrom(float (&f)[256][256]) {
    for(int i=0; i<256; i++) {
      for(int j=0; j<256; j++) {
        this->f[j][i]=f[j][i];
      }
  ...
  void add(Mat256x256float& a, Mat256x256float& b) {
    for(int i=0; i<256; i++) {
      for(int j=0; j<256; j++) {
        int tmp=int(a.f[i][j])+int(b.f[i][j]);
        this->f[i][j]=(tmp&0xFF);
      }
    }
  }
...
struct Arr256x64i32 {
  ...
  Arr256x64i32(const Mat256x256float& mat) {
    for(int j=0; j<256; j++) {
      for(int i=0; i<64; i++) {
        d[j][i] = ((uint32_t(uint8_t(mat.f[j][i  + 192]))) << 24) |
            ((uint32_t(uint8_t(mat.f[j][i + 128]))) << 16) |
            ((uint32_t(uint8_t(mat.f[j][i  + 64]))) << 8) |
            ((uint32_t(uint8_t(mat.f[j][i ]))) << 0);
      }
    }
  }
  void reduceFNV() {
...
extern void iter_mineBytom(const uint8_t *fixedMessage, uint32_t len, uint8_t nonce[8], uint8_t result[32], bool isGpu, cublasHandle_t handle);
...
extern BytomMatListGpu* matListGpu_float;
